...
void CMatrix::setRow(uint16_t row)
{
    m_row = row;
}
void CMatrix::setColumn(uint16_t column)
{
    m_column = column;
}
...
RC_t CMatrix::printMatrix()
{
    for (int rowIdx = 0; rowIdx < m_row; rowIdx++)
    {
        for (int colIdx = 0; colIdx< m_column; colIdx++)
        {
            std::cout << this->getBuffer(rowIdx, colIdx) << '\t';
        }
        std::cout << std::endl;
    }
    return RC_SUCCESS;
